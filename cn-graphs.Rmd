...
```{r}
cn_az <- glm.nb(p_az_o20 ~ cn+t_az_o20+attend5+sdo+q38_10rc+cphavg+evil+white+pid7f+age+female+ed+reltrad, 
                   data = O20_az, weight=wgt)
summary(cn_az)

interact_plot(cn_az, pred = cn, modx = t_az_o20, 
              int.width=.76, interval = TRUE,
              legend.main = "") +
  theme_minimal() +
  theme(text=element_text(family="G", size=12),
...
```{r}
cph_az <- glm.nb(p_az_o20 ~ cphavg*t_az_o20+attend5+sdo+q38_10rc+cn+evil+white+pid7f+age+female+ed+reltrad, 
                   data = O20_az, weight=wgt)
summary(cph_az)
